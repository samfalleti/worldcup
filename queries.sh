...
echo "$($PSQL "select winner.name from games join teams as winner on games.winner_id=winner.team_id where year=2014 and round='Eighth-Final' union select loser.name from games join teams as loser on games.opponent_id=loser.team_id where year=2014 and round='Eighth-Final' order by name")"
